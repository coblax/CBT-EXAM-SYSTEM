...
const recoveryHelperPath = path.join(projectRoot, 'tests', 'e2e', 'helpers', 'recovery-fixture.php');
...
function printSkip(messageLines) {
    console.log('PLAYWRIGHT RECOVERY FLOW SKIPPED');
...
function callRecoveryHelper(action, payload) {
    const args = [recoveryHelperPath, String(action || 'fixture')];
    ...
    if (result.error || result.status !== 0) {
        ...
        const detail = stderr || stdout || String(result.error && result.error.message ? result.error.message : 'Helper recovery gagal dijalankan.');
    ...
    try {
        ...
        console.error('Gagal membaca payload helper recovery: ' + String(error && error.message ? error.message : error));
        process.exit(1);
    }
}

function ensureSharedOutputDir() {
    fs.mkdirSync(sharedOutputDir, {
        recursive: true,
        mode: 0o777
    });
    ...
    try {
        ...
        // Best effort only. Some hosts may reject chmod when directory is already managed.
    }
}
...
function relaxSharedOutputPermissions() {
    ...
    while (queue.length > 0) {
        ...
        try {
            ...
            // Best effort only. Keep the runner moving even if chmod is rejected.
        }
...
if (missingLibraries.length > 0) {
    printSkip([
        ...
        'Pasang dependency OS browser terlebih dahulu, lalu jalankan ulang flow check recovery.'
    ]);
    process.exit(0);
}
...
printFlowHeader('Recovery Flow Check Bootstrap', [
    'Base URL: ' + String(process.env.CBT_E2E_BASE_URL || '(belum diisi)'),
    'Browser Path: ' + sharedBrowsersPath,
    'Output Dir: ' + sharedOutputDir,
    'Scenario Filter: ' + (process.argv.slice(2).join(' ') || 'semua skenario recovery'),
]);

const resetPayload = callRecoveryHelper('reset');
if (!resetPayload || resetPayload.ok !== true) {
    console.error('Preflight reset recovery gagal.');
    process.exit(1);
}

printFlowHeader('Preflight Reset', [
    'Reset login session recovery student: selesai',
    'Reset attempt/ui_state/runtime/security/cache fixture recovery: selesai',
    'Deleted attempt count: ' + String(resetPayload.deleted_attempt_count || 0),
    'Fixture: ' + String(resetPayload.fixture && resetPayload.fixture.exam_title ? resetPayload.fixture.exam_title : 'TEST Recovery Fixture'),
]);

const tokenPayload = callRecoveryHelper('global_token');
if (!tokenPayload || tokenPayload.ok !== true) {
    console.error('Preflight token recovery gagal.');
    process.exit(1);
}
...
const result = spawnSync(
    ...
    ['test', 'tests/e2e/recovery-persistence.spec.js', ...process.argv.slice(2)],
    {
        stdio: 'inherit',
        env: {
            ...process.env,
