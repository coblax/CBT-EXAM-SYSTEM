...
final class CBT_Admin_Users_Service
{
    ...
    {
        $required = ['name', 'email', 'nisn', 'username', 'password', 'role'];
        foreach ($required as $column) {
            if (!in_array($column, $header, true)) {
                return new WP_Error('missing_header', 'Kolom wajib belum lengkap. Pastikan minimal ada: name, email, nisn, username, password, role.');
            }
        }
     ...
     */
    public static function upsert_user_from_row(array $row, array &$import_lookup = []): string
    {
        ...
        $foto = esc_url_raw((string) ($row['foto'] ?? ''));
        ...
        if ($user_id > 0) {
            ...
            self::register_user_import_lookup($import_lookup, $user_id, $email, $username, $name !== '' ? $name : $username);
            return 'updated';
        }
    ...
    /**
     * @return array{email:string,username:string,display_name:string}
     */
    ...
    private static function resolve_user_import_existing_id(string $email, string $username, array &$lookup): int
    {
        ...
        if ($user_id > 0) {
            self::register_user_import_lookup($lookup, $user_id, $email, $username, '');
            return $user_id;
        }
     ...
     * @param array<string,mixed> $lookup
     */
    private static function register_user_import_lookup(array &$lookup, int $user_id, string $email, string $username, string $display_name): void
    {
        if ($user_id <= 0) {
            return;
        }
